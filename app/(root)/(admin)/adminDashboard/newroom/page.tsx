...
import { COUNTRY_API, IMGBB_API } from "@/types/SecretKey";
...
export default function NewRoom() {
  ...
  useEffect(() => {
    ...
    const fetchLocations = async () => {
      if (selectedCountry) {
        try {
          console.log(`Fetching locations for country: ${selectedCountry}`);
          const res = await fetch(
            `https://api.countrystatecity.in/v1/countries/${selectedCountry}/states`,
            {
              method: "GET",
              headers: {
                "X-CSCAPI-KEY": COUNTRY_API,
    ...
    fetchLocations();
  }, [setCities, searchTerm, selectedCountry]);
  ...
  const onCreateCategory = async (
    ...
    if (isCreatingCategory) {
      try {
        ...
        toast({
          title: "Something went wrong!",
          description: "Your category was not successfully created.",
          variant: "default",
        });
      }
    } else {
